...
echo
ffmpeg -i ${source} -f webm -c:v libvpx-vp9 -b:v 0 -crf 23 -pix_fmt yuv420p -threads 8 -speed 4 -vf scale=iw:ih -acodec libopus -b:a 64k ${target}/video.webm
echo
ffmpeg -i ${source} -vf "thumbnail,scale=720:1280" -qscale:v 4 -frames:v 1 ${target}/poster.jpg
